...
### Текст для диктора:
> Приветствую, коллеги. В третьем выпуске архитектурной серии мы детально исследуем прикладной каркас и инструментарий разработчика платформы «Турбаза».
>
...
>
> Фреймворк «Турбазы» полностью переворачивает эту модель. В нашей архитектуре приложение пишется как автономный локальный модуль. Разработчик оперирует привычными классами TypeScript, декораторами и SQL-запросами, а фреймворк автоматически гарантирует изоляцию, безопасность и peer-to-peer синхронизацию. Давайте разберем его внутреннее устройство.
...
<div class="slide-content">
  <div class="slide-header">
    ...
    <p class="slide-subtitle">Фундаментальный закон суверенной безопасности данных в экосистеме Турбазы</p>
  </div>
...
graph LR
    ...
    subgraph WriteOperation["ЗАПИСЬ: Write-Self (Строго изолировано)"]
        ...
        App1 -->|Вызов публичного метода @Api()| SafeGateway["API Путешествий &rarr; Валидация &rarr; Запись ✅"]
    end
        </div>
      </div>
    </div>
    ...
    <div class="content-panel">
      <div class="content-cards-stack">
        <div class="content-card border-lime">
          <h4>Суверенитет пользователя над своими данными</h4>
        ...
        <div class="content-card">
          ...
          <p>Ни одно приложение не имеет права напрямую модифицировать таблицы другого вендора. Запись чужих данных возможна только через вызовы публичных методов <code>@Api()</code> приложения-владельца.</p>
        </div>
      </div>
    </div>
  </div>
...
>
> В традиционных операционных системах приложения заперты в «силосных башнях»: чтобы органайзер увидел бронирование из туристического приложения, разработчики должны писать сложные API-интеграции через удаленные серверы.
>
...
<div class="slide-content">
  ...
  <div class="slide-body grid-2col">
    <div class="visual-panel">
      <div class="content-cards-stack">
        <div class="code-block-card">
          <div class="code-header">Объявление публичного API микросервиса (@airline/topics)</div>
          <pre><code>@Injected()
    ...
    <div class="content-panel">
      <div class="content-cards-stack">
        <div class="content-card border-lime">
          ...
          <p>Декоратор <code>@Api()</code> объявляет публичный метод сервиса, доступный для вызова другими приложениями через межпесочный RPC-мост.</p>
        </div>
        <div class="content-card">
...
### Текст для диктора:
> Единственный легитимный способ изменить данные другого приложения — вызвать его публичный метод, помеченный декоратором @Api.
>
...
>
> Главное архитектурное достоинство нашего DI — автоматическая контекстная изоляция. Все зависимости, инжектированные в сервис, бесшовно связываются с текущей транзакцией базы данных. Если операция падает с ошибкой, весь контекст транзакции откатывается атомарно.
...
<div class="slide-content">
  <div class="slide-header">
    ...
    <p class="slide-subtitle">Статически проверяемый DSL на TypeScript: защита от инъекций и мощь реляционного SQL</p>
  </div>
  ...
  <div class="slide-body grid-2col">
    <div class="visual-panel">
      <div class="content-cards-stack">
        <div class="code-block-card">
          <div class="code-header">Полнофункциональный Tarmaq DSL запрос в Турбазе</div>
          <pre><code>const qTask = Q_AIRLINE_TASKS.Task;
const urgentTasks = await this.taskDao.find({
  SELECT: {
    id: qTask.actorRecordId,
    title: qTask.name,
    priority: qTask.urgency
  },
  FROM: [qTask],
  WHERE: AND(
    qTask.urgency.greaterThanOrEquals(Urgency.HIGH),
    qTask.status.notEquals(TaskStatus.COMPLETED)
  ),
  ORDER_BY: [qTask.dueDate.ASC()]
});</code></pre>
        </div>
      </div>
    </div>
    ...
    <div class="content-panel">
      <div class="content-cards-stack">
        ...
        <div class="content-card">
          <h4>Поддержка всей мощи реляционной алгебры</h4>
          <p>DSL поддерживает агрегатные функции, подзапросы, оконные операции, многоуровневые JOIN и проекции <code>Y</code> без компромиссов в выразительности.</p>
        </div>
      </div>
    </div>
  </div>
...
>
> Вместо ненадежного склеивания сырых SQL-строк разработчик оперирует сгенерированными объектами QEntity и логическими предикатами. Любая синтаксическая ошибка или несовпадение типов данных пресекается компилятором на этапе сборки.
>
> При исполнении Tarmaq транслирует граф условий в оптимизированные параметризованные команды SQLite. Это дает стопроцентную защиту от SQL-инъекций и сохраняет доступ ко всей мощи реляционной алгебры.
...
<div class="slide-content">
  <div class="slide-header">
    <div class="slide-tag">НЕПРЕРЫВНОСТЬ СЕРВИСА &middot; БЕСШОВНЫЕ ОБНОВЛЕНИЯ</div>
    <h2 class="slide-title">ЭВОЛЮЦИЯ МОДЕЛЕЙ ДАННЫХ БЕЗ ДАУНТАЙМА</h2>
    <p class="slide-subtitle">Версионирование DDL, инъекция патчей схем и работа разнородных версий приложений</p>
  </div>
  ...
  <div class="slide-body grid-2col">
    <div class="visual-panel">
      <div class="mermaid-diagram-card">
        <div class="mermaid">
graph LR
    subgraph ClientA["Узел А: Приложение v1.0"]
        SchemaV1["Схема v1: Колонка name"]
    end

    subgraph ClientB["Узел Б: Приложение v2.0"]
        SchemaV2["Схема v2: name + urgency + tags"]
    end

    subgraph SQLite["Единая СУБД узла: Динамическая адаптация"]
        Patch["Патч схемы: ALTER TABLE ADD COLUMN"]
        Compat["Обратная совместимость: Дефолтные значения & Виртуальные представления"]
    end

    ClientB -->|Инъекция новой схемы| Patch
    Patch --> SQLite
    SchemaV1 -.->|Чтение через адаптер совместимости| SQLite
        </div>
      </div>
    </div>
    ...
    <div class="content-panel">
      <div class="content-cards-stack">
        <div class="content-card border-lime">
          <h4>Вызов децентрализованной эволюции</h4>
          <p>В серверной архитектуре администратор останавливает сервис и накатывает миграцию. В сети из миллионов автономных узлов принудительно обновить всех одновременно невозможно.</p>
        </div>
        <div class="content-card">
          <h4>Атомарная инъекция патчей DDL</h4>
          <p>При установке новой версии модуль приносит упакованный патч схемы. Ядро выполняет инкрементальное расширение колонок без блокировки работы других приложений.</p>
        </div>
        <div class="content-card">
          <h4>Семантическая обратная совместимость</h4>
          <p>Старые версии приложений продолжают работать с базой через адаптер совместимости, игнорируя новые поля, пока пользователь не решит обновить интерфейс.</p>
        </div>
      </div>
    </div>
  </div>
...
### Текст для диктора:
> Как обновлять структуры таблиц, когда приложение развернуто на миллионах независимых клиентских устройств?
>
> В облачных монолитах для этого объявляют технические перерывы и запускают миграции. В децентрализованной сети «Турбазы» это недопустимо: разные пользователи могут работать на разных версиях приложения одновременно.
>
> Фреймворк поддерживает непрерывную эволюцию схем. При запуске новой версии ядро атомарно применяет инкрементальные DDL-патчи. Старые версии клиентов продолжают работать с таблицами в режиме обратной совместимости. Платформа гарантирует стабильность хранения данных на протяжении многих лет без потери истории.
...
graph TB
    ...
    Store["🌐 Реестр репозиториев (Ветка / P2P Сеть)"]
    ...
    <div class="content-panel">
      <div class="content-cards-stack">
        <div class="content-card border-lime">
          <h4>Суверенная дистрибуция без цензуры</h4>
          <p>Приложения не зависят от зарубежных магазинов приложений (App Store / Google Play). Пакеты дистрибутируются через суверенные узлы-ветки и P2P-сеть.</p>
        </div>
        <div class="content-card">
    ...
    <div class="content-panel">
      <div class="content-cards-stack">
        <div class="content-card border-cyan">
          ...
          <p>В следующем выпуске мы разберем <strong>Архитектуру узла «Ветка» (Branch Node)</strong>: конвейер данных высокой надежности, трехфазную цепочку PersistentQueue &rarr; ConcurrentMap &rarr; FileSystem, Time-Traveling и обслуживание публичных реестров.</p>
        </div>
        <div class="content-card">
...
>
> Framework и SDK платформы «Турбаза» предоставляют инженерам законченный инструмент для создания распределенных суверенных приложений нового поколения.
>
...
>
> В следующем, четвертом выпуске мы поднимемся на уровень выше и исследуем архитектуру узла «Ветка» — периферийного координатора гарантированной надежности. Спасибо за внимание, увидимся в выпуске номер четыре.
