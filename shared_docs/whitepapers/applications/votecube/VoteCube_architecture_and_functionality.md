...
export class Idea extends AirEntity {
    ...
    // Каждый участник создает собственный независимый Agreement (со своим UserId).
...
Блоки эпох синхронизируются между Веткой и Листьями как стандартные сущности **AIRport (`AirEntity`)**. Ветка публикует `EpochBlock` в хранилище вопроса, подписывая запись своим `BranchId`:
...
* **Шлюзы Веток и Ветвей с адаптивным древовидным разделением:** Ветки (муниципальные) и Ветви (кооперативные) функционируют не как серверы приложений, а как высокопроизводительные шлюзы данных. Однако в рамках систем «КубГолоса» и «Забота» эти шлюзы берут на себя **дополнительные специализированные функции координации общей информации**:
  * общая информация делится **различными древовидными методами подбора в зависимости от вида информации**:
    1. *Деревья страниц (`IChildRecordPage`)*: структурирование неограниченных связей 1:N (треды реплик, списки случаев) на компактные самозапечатывающиеся хранилища;
...
* **Отказ от монолитного хранилища:** Вся платформа отказывается от идеи хранения всех вопросов в одном файле. Каждый вопрос и его дерево деривативов — это **отдельное микро-хранилище (микро-блокчейн)**.
...
2. Ветка принимает пакет от зарегистрированного `UserId` и атомарно выполняет коррекцию в памяти:
   $$\text{CurrentTotal} \leftarrow \text{CurrentTotal} - \text{Vote}_{\text{old}}(\text{UserId}) + \text{Vote}_{\text{new}}(\text{UserId})$$
...
1. **Страницы (Pages / `IChildRecordPage`)**:
   * Разрабатываются в «Заботе» и базовой схеме для деления связей 1:N, предотвращения раздувания хранилищ и построения самозапечатывающихся B-деревьев.
...
* **Исключение рассинхронизаций в офлайне:** Любой голос, отправленный смартфоном после недели нахождения вне сети, гарантированно и бесконфликтно применяется к хранилищу.

### 6.2. Квотирование добавления факторов (Leaky Bucket)
Для предотвращения спама и замусоривания базы факторов Ветка применяет строгий алгоритм капельного счетчика к каждому `UserId`:
...
* **Превентивная подсказка (In-Flight Suggestion)**: еще до фиксации транзакции в хранилища интерфейс Листа отображает предупреждение: *«Похожая проблема уже обсуждается в вашем районе (семантическое совпадение 87%). Присоединиться к существующей дискуссии?»*. Это снижает появление дублей на 60–80%.
...
### 6.5. Назначение сущности Actor в AIRport и трехуровневая анонимность на Ветке
* **Техническое назначение `Actor`:**
  * В архитектуре AIRport сущность `Actor` используется исключительно для технической идентификации того, **через какое приложение и на каком конкретно аппарате (устройстве)** было выполнено изменение данных (иначе в системе использовался бы лишь идентификатор пользователя `UserId`).
  * Первичный ключ любой записи в хранилище формируется триплетом: `(RepositoryId, ActorId, ActorRecordId)`. Это гарантирует отсутствие коллизий ключей при параллельном создании записей одним и тем же пользователем с нескольких физических устройств или из разных приложений.
* **Механизмы анонимности (на уровне регистрации на Ветке):**
  * Сущность `Actor` не используется для обеспечения анонимности. Приватность и защита личности в платформе реализуются непосредственно на **уровне регистрации пользователей на Ветке**:
    1. *Уровень 1 (Branch ID / Локальная анонимность):* базовая регистрация на районной Ветке при нахождении в локальной сети района без предоставления персональных данных;
    2. *Уровень 2 (Групповая анонимность / Жеребьевка):* криптографическое доказательство гражданства и территориального ценза через доверенную группу соседей (офлайн-жеребьевка). Ветка видит, что номер принадлежит легитимной группе $N$, но сопоставить его с конкретным человеком технически невозможно;
    3. *Уровень 3 (Персональная верификация):* открытый режим с персональной репутацией для признанных экспертов и официальных лиц.
  * Гражданин самостоятельно выбирает уровень анонимности при волеизъявлении или публикации случаев, опираясь на инфраструктурные механизмы Ветки.
...
2. **Лимиты скорости и квоты Ветки (`user_factor_quotas`)**:
   - Шлюз Ветки отслеживает частоту транзакций от конкретного `UserId`. Попытка высокочастотного спама дельтами приводит к автоматическому замораживанию mTLS-сессии устройства без влияния на легитимных соседей.
...
### 8.2. Разделение связей «Один-ко-Многим» (1:N) в децентрализованных хранилищах через «Самозапечатывающиеся страницы» (`IChildRecordPage`)

#### 8.2.1. Проблема неограниченных связей 1:N в децентрализованных P2P-хранилищах
...
В пиринговых децентрализованных архитектурах (AIRport / «Турбаза») **Хранилище (Repository)** является неделимой единицей хранения, криптографической верификации, контроля доступа и синхронизации транзакций:
1. **Раздувание хранилища (Repository Bloat) и провал первичной синхронизации**: Если бы родительская сущность (`Topic`, `Situation` или тред `Case`) хранила прямую неограниченную коллекцию дочерних записей (`OneToMany`) в рамках одного хранилища, его журнал транзакций (WAL) разрастался бы до десятков гигабайт. При первой синхронизации мобильный клиент (Лист) был бы вынужден скачивать весь массив истории до того, как сможет отобразить интерфейс, что делает работу на мобильных устройствах и в офлайне невозможной.
2. **Конфликты конкурентной записи (Write Contention)**: Одновременное добавление записей сотнями распределенных участников в одно хранилище вызывает лавину конфликтов сериализации и огромные накладные расходы на мерж дельт.
3. **Разрушение неизменяемости и кэширования**: Любая новая дочерняя запись модифицирует хэш состояния родительского хранилища, делая невозможным перманентное кэширование данных на промежуточных узлах и устройствах.

#### 8.2.2. Архитектура «Самозапечатывающейся страницы» (`IChildRecordPage`) как Агрегативного Хранилища
Для устранения проблемы раздувания данных платформой введен универсальный паттерн пагинации, где связь 1:N разбивается на цепочку дискретных ограниченных страниц:
* **Страница как Агрегативное Хранилище (Aggregate Repository):**
  По своей природе страница `IChildRecordPage` (`TopicPage`, `TopicSituationPage`, `SituationPage`, `ReplyPage`) является **агрегативным хранилищем**, которое аккумулирует не сами полные дочерние сущности, а облегченные проекции и ссылки на объекты из множества других независимых хранилищ.
* **Почему используется стандартный механизм хранилищ AIRport?**
  Страницы оформлены как полноценные хранилища AIRport исключительно потому, что **это унифицирует их обработку на Листах**:
  - Клиентский узел (Лист) использует для страниц точно такой же типовой конвейер локальной репликации в SQLite / WASM, криптографической верификации подписей, CAS-кэширования и реактивных запросов (`live queries`), как и для любых других сущностей;
  - Отпадает необходимость проектировать специализированные сетевые API или нестандартные протоколы списков и пагинации — страницы реплицируются, версионируются и синхронизируются через единый стандартный механизм журналов дельт (WAL);
  - Обеспечивается полная оффлайн-доступность списочных представлений без дополнительных адаптеров.
* **Структура страницы (две взаимосвязанные зоны)**:
  1. **Зона А: Полезная нагрузка записей (`Payload List`)**:
     * Фиксированный массив облегченных проекций дочерних сущностей (`children: ChildRecord[]`) с мягким лимитом около 1000 записей ($K \approx 1000$).
  ...
  2. **Зона Б: Ссылки на дочерние страницы (`Child Branch Links`)**:
     ...
     * При запечатывании текущей страницы порождается новая страница-хранилище, куда направляются все последующие записи (Append-Only).
* **Мягкий лимит (~1000 записей)**:
  ...
  * Принципиально то, что объем страницы остается строго ограниченным константным размером $O(1)$, предотвращая раздувание хранилища.
...
* Семейный контур использует **единый зашифрованный хранилище**, поскольку объем данных семьи невелик (сотни записей), а сквозное шифрование и ротация ключей группы (Keyring) в рамках одного хранилища работают надежнее и с меньшими накладными расходами.
...
#### 8.2.6. Отношение Листа и Ветки к страницам IChildRecordPage: управление Веткой и ленивое чтение Листом
* **Исключительное управление страницами Веткой:**
  Лист **никогда напрямую не модифицирует** хранилища `IChildRecordPage` (`TopicPage`, `TopicSituationPage`, `SituationPage`, `ReplyPage`) и ничего в них не записывает. Всеми страницами управляет исключительно Ветка, отвечающая за соответствующий `Topic` (или группу топиков).
* **Создание дочерней сущности Листом:**
  Когда пользователь создает новую сущность (например, `Situation`), Лист формирует и подписывает своим `ActorId` локальное хранилище самой сущности и отправляет её на Ветку вместе со ссылкой на родительский объект (`parent: Topic`).
* **«Листовая» (списочная) информация в `children` (Shallow Children / List View Projection):**
  Хранилище страницы хранит лишь **«листовую» (списочную / превью) информацию** по находящимся в ней дочерним объектам: идентификатор, название (`title`/`name`) и минимальный набор колонок, строго необходимых для отображения определенного вида списка (list view / feed preview) в пользовательском интерфейсе Листа (например, тип сущности, статус или краткий сниппет). Полная сущность с её детальным описанием, историей и вложенными коллекциями сохраняется исключительно в собственном независимом хранилище. Это позволяет пользовательскому интерфейсу Листа мгновенно визуализировать список элементов без необходимости загрузки индивидуальных хранилищ каждого дочернего объекта.
* **Синхронизация при обновлении дочерних объектов (Update Propagation):**
  Поскольку страницы запечатлевают название и визуальные поля дочерних объектов, при редактировании или обновлении хранилищ самих дочерних объектов (например, когда автор изменяет название Ситуации, статус Случая или текст Реплики), соответствующее хранилище страницы **также получает обновление (WAL-дельту)** от Ветки. Это гарантирует, что списочный вид в пользовательском интерфейсе всегда отображает актуальные заголовки и метаданные без рассинхронизации между списком и самими объектами.
* **Переход на новые страницы и B-Tree страниц:**
  При переполнении страницы (~1000 записей) Ветка создает новую страницу `TopicSituationPage` в новом хранилище. В журнал изменений хранилища родительского `Topic` (в `situationPages`) или родительской страницы (в `childPages`) Ветка добавляет ссылку на это хранилище в виде **пустого объекта-заглушки только с идентификатором**. Корневые страницы для каждого `Situation_Type` находятся в `Topic.situationPages`, а дальнейшие страницы связываются через `childPages`/`parentPage`.
* **Постраничная навигация Листа:**
  Поскольку сущность реализует `IChildRecordPage`, Лист знает, что в `Topic.situationPages` находятся лишь идентификаторы страниц. При открытии интерфейсного списка Лист последовательно запрашивает первую страницу, отображает облегченные карточки `children` и по мере скролла/навигации запрашивает следующие страницы из `situationPages` или `childPages`.
* **Мульти-типизация страниц через `Situation_Type`:**
  Параметризация `IChildRecordPage<Situation_Type, Topic, Situation>` позволяет Ветке параллельно вести несколько потоков страниц для одного топика, системно разделяя общие ситуации «КубГолоса» (`GENERAL`) и автоматические ситуации из «Заботы» без смешивания их в общем списке. Текстовый поиск при этом выполняется через подсистему **TreeSearch** без задействования страниц.

### 8.3. Поиск и кластеризация по тегам (Двухуровневая модель)
Для обеспечения быстрого поиска по тегам (`#детскийсад`, `#парковка`, `#экология`) в условиях сотен тысяч распределенных страниц-хранилищ применяется двухуровневая схема:
...
2. **Индексные страницы тегов в AIRport (`TagPage` как `IChildRecordPage`)**:
   * Для каждого тега формируется собственное дерево страниц-хранилищ, содержащих криптографические ссылки на релевантные Ситуации и Случаи.
...
1. **Модель суверенного самохранения (Self-Custody)**:
   - Все персональные параметры, история просмотров и закрытые ключи хранятся **исключительно на Листе (устройстве самого гражданина)** в локально зашифрованном хранилище SQLite.
