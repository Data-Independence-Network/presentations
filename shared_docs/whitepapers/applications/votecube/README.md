...
> • [**`VoteCube_architecture_and_functionality.md`**](VoteCube_architecture_and_functionality.md) — Нормативная архитектурная спецификация (1.0)  
> • [**`VoteCube_Ecosystem_Impact_Guide.md`**](VoteCube_Ecosystem_Impact_Guide.md) — Руководство по социально-экономическому воздействию на 7 уровнях общества  
> • [**`AGENTS.md`**](AGENTS.md) — Инженерный регламент и системные инварианты VoteCube  
...
* **Интеграция со внешними поисковыми системами**:
  ...
  - Пользователь переходит из поисковика к целевому топику, после чего Лист запрашивает журнал изменений соответствующего микрохранилища через шлюз Ветки в свою локальную базу.
...
### 4.2. Самозапечатывающиеся страницы (`IChildRecordPage`) как Агрегативные Хранилища
Прямые реляционные связи 1:N в децентрализованных P2P-хранилищах приводят к раздуванию хранилищ, падению производительности мобильных устройств и конфликтам синхронизации. В «КубГолосе» эта проблема решена через платформенный интерфейс `IChildRecordPage`:
* Страницы `TopicPage`, `TopicSituationPage`, `SituationPage` представляют собой **агрегативные микрохранилища AIRport**, аккумулирующие облегченные проекции объектов из других независимых хранилищ и ограниченные объемом ~1000 записей.
* **Унификация обработки на Листах:** Механизм хранилищ применен для страниц именно потому, что это унифицирует их обработку на клиентских устройствах (Листах). К страницам применяется стандартный конвейер репликации WAL-дельт в SQLite / WASM, CAS-кэширования и реактивных запросов (`live queries`), исключая потребность в кастомных API для списков.
...
### 4.5. Управление страницами `IChildRecordPage` Веткой и ленивая навигация Листа
В архитектуре AIRport страницы коллекций децентрализованы:
* **Исключительное управление Веткой:** Лист никогда напрямую не модифицирует страницы `IChildRecordPage` (`TopicSituationPage`, `TopicPage`, `SituationPage`). Всеми страницами управляет исключительно Ветка, отвечающая за соответствующий Топик.
* **Создание сущности Листом:** Лист создает и подписывает своим `ActorId` локальное хранилище сущности (`Situation`) и отправляет его на Ветку с ссылкой на `parent: Topic`.
* **«Листовая» информация (Shallow Children):** Ветка находит открытую страницу `TopicSituationPage` нужного типа (`Situation_Type`) и добавляет в массив `children` лишь «листовую» (списочную / превью) информацию: идентификатор и минимальные колонки для визуализации определенного вида списка в UI Листа (название/заголовок, краткий текст). Полная сущность хранится в собственном независимом хранилище.
* **Синхронизация обновлений:** Поскольку страницы запечатлевают названия дочерних объектов, при обновлении или редактировании хранилищ самих дочерних объектов соответствующее хранилище страницы также получает обновление (WAL-дельту) от Ветки, гарантируя актуальность названий в UI-списках.
* **Иерархия страниц (B-Tree) при заполнении (~1000 записей):** Ветка создает новую страницу в отдельном хранилище и добавляет пустую запись-заглушку только с идентификатором в журнал родительского `Topic.situationPages` (корневые страницы) или в `childPages` родительской страницы.
* **Постраничная навигация Листа:** Зная контракт `IChildRecordPage`, Лист запрашивает первую страницу из `situationPages`, отображает облегченный список `children`, а последующие страницы подгружает по мере навигации из `situationPages` или `childPages`. Текстовый поиск при этом выполняется через подсистему **TreeSearch** без использования страниц.


### 4.6. Назначение сущности Actor и уровни анонимности на Ветке
* **Назначение `Actor`:** Сущность `Actor` в AIRport предназначена исключительно для идентификации того, через какое приложение и на каком аппарате сделано изменение (`(RepositoryId, ActorId, ActorRecordId)`), исключая коллизии первичных ключей при параллельной синхронизации устройств пользователя.
* **Анонимность на Ветке:** Конфиденциальность не использует `Actor`, а обеспечивается на уровне регистрации пользователя на районной Ветке:
  1. *Уровень 1 (Branch ID):* базовая анонимность в локальной сети района;
  2. *Уровень 2 (Групповая анонимность / Жеребьевка):* подтверждение гражданства и ценза проживания через офлайн-группу без деанонимизации личности;
  3. *Уровень 3 (Персональная верификация):* открытый экспертный режим с репутационной ответственностью.
  Пользователь сам выбирает уровень анонимности исходя из контекста опроса.
...
* Все персональные данные и закрытые ключи хранятся **исключительно на Листе (устройстве гражданина)**. Гражданин является единственным контролером своего хранилища (модель Self-Custody).
...
* Построение **единого комплексного демонстрационного приложения (Single-Bundle App)**, объединяющего КубГолос, Заботу и Деловой:
  ...
  4. **Вкладка 4: «Профиль / DIN»** — верификация территориального резидентства, баланс экспертных очков, тумблер анонимного режима (Branch ID / Жеребьевка) и начисление дохода по смарт-контракту `share(...)`.
