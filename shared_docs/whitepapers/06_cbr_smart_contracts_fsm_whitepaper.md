...
4. [Математический аппарат минимальных FSM-контрактов и прикладные сценарии](#4-математический-аппарат-минимальных-fsm-контрактов-и-прикладные-сценарии)
   ...
   - 4.3. Рекурсивное распределение выручки разработчиков и 90% кешбэк пользователю
...
Мы выражаем искреннюю поддержку инициативе Банка России. Настоящий документ подготовлен независимой исследовательской группой платформы **«Турбаза»** — отечественного архитектурного проекта распределенных суверенных вычислений. Центр клиентского движка («Лист») открыт и доступен сообществу в рамках свободного репозитория **AIRport (Application Information Repository Port)**:  
...
* **Финансовые директивы:** `set(amount, account)`, `add(amount, account, target)`, `share(target, ratio, source, ...)`.
...
В современной разработке программные продукты строятся из готовых модулей. Если приложение разработчика $Б$ использует библиотеку разработчика $А$, распределение дохода от монетизации задается в контракте декларативно:
```
В = share(Б, 0.5, А)
```
Контракт $В$ может быть рекурсивно включен в более сложный продукт разработчика $Д$, использующего также модуль $Г$:
```
Е = share(Д, 0.6, В, 0.2, Г)
```

**Модель прямого AdTech-кешбэка гражданам (90% Revenue Sharing):**
В цифровой экономике внимание пользователя монетизируется через рекламу. В платформе «Турбаза» контракт показа рекламы возвращает львиную долю выручки непосредственно человеку:
```
share(Я, 0.9, Е)
```
* **90%** платежа рекламодателя мгновенно и прозрачно зачисляется на кошелек пользователя $Я$ в Цифровых рублях;
* **10%** распределяются между разработчиками интерфейса и библиотек по формуле композитного контракта $Е$.

Это устраняет посредников-монополистов и создает у граждан беспрецедентный интерес к использованию платформы Цифрового рубля.
...
2. **Прямой кешбэк от AdTech-рынка (`share(Я, 0.9, Е)`):** Возврат гражданину до 90% стоимости просмотра рекламы в Цифровых рублях.
