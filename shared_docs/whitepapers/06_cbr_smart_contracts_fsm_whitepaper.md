...
3. **Бестерминальность, локальный FSM и долговые расписки (IOU):** Кассе столовой не требуется POS-терминал: расчет идет через официальное приложение Банка России в контролируемом браузере «Турбазы». При обрыве связи на смартфоне школьника формируется подписанная (63-ФЗ) долговая расписка (IOU-дельта); локальная копия FSM во впускном приложении кассы автономно валидирует кэш баланса, интервал `onceIn` и лимит 300 ₽. В замкнутой школьной среде взаимное доверие позволяет осознанно принять риск отложенного клиринга ради непрерывности питания детей. При восстановлении связи пул расписок финализируется в ядре ЦБ (ч. 10 ст. 5 161-ФЗ).
...
2. **Гарантия авто-возврата по таймауту и офлайн-расписки:** Если заказ не выдан в срок, срабатывает детерминированный переход `onceIn(T_timeout, refund(Покупатель))`. При обрыве связи участники через прямые P2P-потоки данных оформляют сделку долговыми расписками (IOU) на базе кэша балансов и проверки условий локальным FSM.
...
4. **Офлайн-устойчивость с поддержкой долговых расписок (IOU):** Расчеты при обрыве связи через локальный кэш балансов, проверку условий копией FSM во впускном приложении ЦБ и взаимное P2P-подписание долговых расписок с отложенным клирингом.
</div>
...
1. **Реестр доверенных оракулов:** Государственные ГИС (ЕГРН, ЕИС Закупки, ГИБДД, ГИС ЭПД, ГИС ЖКХ, АСК НДС), биржевые системы (MOEX, СПбМТСБ, курсы ЦБ) и P2P-консенсус в микро-цепях «Турбазы» (взаимный консенсус подписей сторон прямо на аппаратах под контролем версий платформы местной Веткой);
