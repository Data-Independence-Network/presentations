...
```
*Параметры формулы:* `onceIn(86400, ...)` — квантование расхода 1 раз в 24 часа; `lte(300, ...)` — лимит до 300 ₽; `or(А, Б)` — целевые заполнители (**А** — школьный завтрак, **Б** — комплексный обед).
...
3. **Бестерминальность, локальный FSM и осознанный IOU-офлайн:** Кассе столовой не требуется POS-терминал: расчет идет через приложение Банка России в браузере «Турбазы». Переход к долговым распискам (IOU) происходит строго не автоматически: при сбое сети платформа оповещает кассира и школьника о невозможности онлайн-клиринга ЦБ, запрашивая их осознанное согласие на отложенный расчет. Локальная копия FSM на кассе автономно валидирует баланс, интервал `onceIn` и лимит 300 ₽ по формуле контракта, заранее сохраненной в локальном кэше кассы при первом расчете ученика. В замкнутой школьной среде взаимное доверие позволяет принять риск отложенного клиринга ради непрерывности питания детей, а пул подписанных (63-ФЗ) расписок финализируется в ЦБ при восстановлении связи (ч. 10 ст. 5 161-ФЗ).
