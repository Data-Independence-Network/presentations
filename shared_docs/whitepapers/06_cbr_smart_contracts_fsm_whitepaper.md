...
```
*Параметры формулы:* `onceIn(86400, ...)` — квантование расхода 1 раз в 24 часа; `lte(300, ...)` — лимит до 300 ₽; `or(А, Б)` — целевые заполнители (**А** — школьный завтрак, **Б** — комплексный обед). Школьнику не требуется смартфон: доверенная касса столовой выступает исполнителем перехода автомата строго в границах заданных предикатов.

```mermaid
sequenceDiagram
    autonumber
    participant Parent as 👨‍💼 Родитель (Лист)
    participant Child as 🎒 Школьник (NFC-носитель)
    ...
    Parent-->>Child: Привязка предикатного лимита к NFC-карте/браслету
    Child->>Canteen: Выбор питания и прикладывание NFC-носителя
    Canteen->>Canteen: Считывание ID ребенка и формирование перехода (280 ₽ -> Б)
    Canteen->>CBR: Запрос перехода FSM за подписью кассы (ГОСТ/ЭП)
    ...
    Canteen-->>Parent: Синхронизация чека по микро-цепи (Zero-PII)
```
...
3. **Пассивный носитель, доверенная касса и осознанный IOU-офлайн:** Школьнику не нужен смартфон: расчет инициируется касанием карты или браслета на кассе столовой (узел «Лист» платформы «Турбаза»). Касса формирует и подписывает транзакцию перехода FSM. Злоупотребления со стороны кассира исключены: ядро Банка России отклонит любую сумму свыше 300 ₽, попытку списать средства повторно в течение суток или перевод не на счет комбината питания. Переход к долговым распискам (IOU) происходит строго не автоматически: при сбое сети платформа оповещает кассира о невозможности онлайн-клиринга ЦБ, запрашивая осознанное согласие на отложенный расчет. Локальная копия FSM на кассе автономно валидирует баланс, интервал `onceIn` и лимит 300 ₽ по формуле контракта, заранее сохраненной в локальном кэше кассы при первом расчете ученика. В замкнутой школьной среде взаимное доверие позволяет принять риск отложенного клиринга ради непрерывности питания детей, а пул подписанных (63-ФЗ) расписок финализируется в ЦБ при восстановлении связи (ч. 10 ст. 5 161-ФЗ).
...
4. **Криптографическое кэширование FSM-шаблонов в ОЗУ:** Единый физический экземпляр автомата в памяти серверов ЦБ обслуживает миллионы однотипных контрактов по хэшу формулы;
