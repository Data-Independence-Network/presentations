...
<div class="slide-content">
  <div class="slide-header">
    ...
    <p class="slide-subtitle">Двухконтурная модель «Турбазы» и будущей системы смарт-контрактов Банка России: детерминированные механизмы состояний FSM O(1) и Zero-PII</p>
  </div>
  ...
  <div class="slide-body grid-2col">
    <div class="info-panel">
      <div class="key-callout teal">
        ...
        <p>«Турбаза» соединяет национальную платежную систему с суверенным распределенным интернетом данных. <strong>Расчетное ядро ЦБ освобождается от чужого кода и чеков</strong>, сводясь к детерминированному расчетному механизму O(1), а вся бизнес-логика и тайна жизни граждан исполняются на их устройствах.</p>
      </div>
...
### Текст для диктора:
> Здравствуйте! В заключительной части вводного обзора платформы «Турбаза» показано, как устроена справедливая суверенная экономика данных и смарт-контрактов.
>
> Для полноценного функционирования «Турбазы» необходима государственная расчетная система — её логично связать с будущей национальной платформой смарт-контрактов Банка России. В этой презентации представлена версия архитектуры будущих смарт-контрактов, описанная в белой бумаге автора для Банка России: контракт в платежном ядре должен быть не тяжелой программой общего назначения, а сверхнадёжным детерминированным механизмом состояний над цифровыми кошельками.
>
...
>
> В этой презентации на реальных примерах подробно разобрана работа двухконтурной архитектуры: оплата школьного питания, безопасная доставка без терминалов и справедливые выплаты создателям программ.
...
>
> Государственная платежная система не должна повторять эти ошибки. Решение «Турбазы» — свести ядро расчетов к детерминированному расчетному механизму, а прикладную сложность вынести наружу.
  ...
  <div class="slide-body grid-2col">
    <div class="glass-card accent-golden" style="height: 100%; justify-content: space-between;">
      <div>
        ...
        <p class="card-desc">Государственный детерминированный механизм состояний (FSM) O(1) над цифровыми кошельками.</p>
    ...
    <div class="glass-card accent-teal" style="height: 100%; justify-content: space-between;">
      <div>
        ...
        <p class="card-desc">Автономная распределенная среда реляционных хранилищ прямого владения данными на аппаратах участников («Лист»).</p>
...
>
> Первый — внутренний расчетный контур, предложенный в этой концепции для Банка России. Это детерминированный расчетный механизм класса Мили, управляющий кошельками Цифрового рубля. Он оперирует исключительно балансами, моментально производя списания по математическим формулам. В нём нет персональных данных и тяжелого кода.
>
...
<div class="slide-content">
  <div class="slide-header">
    ...
    <p class="slide-subtitle">Кошельки как переменные механизма Мили и библиотека универсальных шаблонов математических функций</p>
  </div>

  <div class="slide-body grid-2col">
    <div class="info-panel">
      <div class="key-callout teal">
        <strong>Формализация механизма Мили:</strong>
      ...
      <div class="fsm-state-pipeline" style="margin-top: 10px;">
        ...
        <div class="fsm-state-row teal">
          ...
          <span class="fsm-state-desc"><code>q_next = &delta;(q_curr, &Sigma;_gost)</code>: чистый функциональный переход состояний O(1)</span>
        </div>
        <div class="fsm-state-row teal">
...
### Текст для диктора:
> Что же такое смарт-контракт в парадигме прямого владения данными? Это детерминированный механизм состояний над цифровыми кошельками.
>
...
<div class="slide-content">
  <div class="slide-header">
    ...
    <p class="slide-subtitle">Исполнение единого экземпляра механизма в ОЗУ ядра по криптографическому хэшу формулы</p>
  </div>
  ...
  <div class="slide-body grid-2col">
    ...
    <div class="glass-card accent-teal" style="height: 100%; justify-content: space-between;">
      <div>
        <div class="card-pill-tag teal">АРХИТЕКТУРА ТУРБАЗЫ: ЕДИНЫЙ ЭКЗЕМПЛЯР В ОЗУ</div>
        <h3 class="card-title">Кэширование по хэшу H(Template)</h3>
        <p class="card-desc">Один экземпляр механизма обслуживает миллионы однотипных контрактов.</p>
        <div class="flow-step-row teal">
          <span class="flow-step-num teal">01</span>
      ...
      <div class="fsm-terminal-card" style="margin-top: auto;">
        ...
        <div class="fsm-terminal-body">
          ...
          <span class="fsm-kw">REGISTRY:</span> 1 экземпляр механизма на 10 000 000 контрактов<br/>
...
>
> Если миллион родителей создают одинаковый контракт на школьное питание или тысячи магазинов запускают доставку, в оперативной памяти исполняется один-единственный общий экземпляр расчетного механизма.
>
    ...
    <div class="info-panel">
      ...
      <div class="scenario-steps-list">
        <div class="flow-step-row teal">
          ...
          <span><strong>Родительский контроль:</strong> родитель задает лимит 300 ₽ и привязывает его к карте или браслету</span>
        </div>
        <div class="flow-step-row teal">
          <span class="flow-step-num teal">2</span>
          <span><strong>Оплата в столовой:</strong> школьник прикладывает карту или браслет к кассе (280 ₽, правило Б)</span>
        </div>
        <div class="flow-step-row teal">
          <span class="flow-step-num teal">3</span>
          <span><strong>Валидация предикатов:</strong> касса проверяет лимит 280 ≤ 300 ₽ и направляет переход в ЦБ</span>
        </div>
        <div class="flow-step-row teal">
          <span class="flow-step-num teal">4</span>
          <span><strong>Мгновенный клиринг:</strong> Банк России зачисляет 280 ₽ столовой ровно за один такт без комиссии</span>
        </div>
      </div>
      ...
      <div class="fsm-state-pipeline">
        <div class="fsm-state-row gold">
          ...
          <span class="fsm-state-desc">Суточный лимит активирован; карта или браслет авторизованы</span>
        </div>
        <div class="fsm-state-row gold">
          <span class="fsm-state-badge">[S1_VERIFY]</span>
          <span class="fsm-state-desc">Проверка кассой: <strong>280 ≤ 300 ₽ ∧ onceIn(86400) ≡ TRUE</strong></span>
        </div>
        <div class="fsm-state-row gold">
      ...
      <div class="glass-card accent-golden" style="height: 100%; justify-content: space-between;">
        <div>
          ...
          <ul class="card-bullets">
            ...
            <li><strong>Пассивный носитель и доверенная касса:</strong> школьнику не нужен смартфон &mdash; расчет идет по карте или браслету; доверенная касса школы проводит платеж строго по предикатам</li>
...
>
> Родитель формирует простой смарт-контракт: тратить не более трехсот рублей, не чаще одного раза в сутки, и только на завтрак или обед. Лимит привязывается к обычному носителю ребенка — школьной карте или силиконовому браслету.
>
> В столовой школьнику не нужен телефон: он просто прикладывает карту или браслет к кассе. Доверенная касса школы считывает идентификатор, проверяет сумму, формирует расчетный переход и направляет его в расчетное ядро. Банк списывает двести восемьдесят рублей на счет школы ровно за один шаг.
>
...
>
> При возникновении спора вся бизнес-логика арбитража, переписка и доказательства остаются в приложениях и микро-цепях «Турбазы». В расчетном механизме хранится лишь адрес универсального шаблона функции, а приложения внешнего контура наполняют его смыслом, направляя решение с подписью медиатора.
...
>
> Модель детерминированных механизмов состояний, описанная в белой бумаге автора, решает эту проблему в корне. Пространство состояний FSM конечно и счетно.
>
  ...
  <div class="slide-body grid-2col">
    <div class="info-panel">
      <div class="key-callout teal">
        ...
        <p>Пройден путь от философии прямого владения своей информацией (Часть 1) и трехзвенной архитектуры шлюзов (Часть 2) к справедливой двухконтурной экономике FSM-контрактов в Цифровом рубле (Часть 3).</p>
      </div>
...
### Текст для диктора:
> Подведем итоги этой вводной обзорной трилогии.
>
> В первой части было обосновано возвращение гражданам прямого владения своей информацией, а во второй — раскрыта анатомия шлюзов, микро-цепей и открытых интерфейсов. В этой презентации представлена завершенная модель справедливой двухконтурной экономики.
>
